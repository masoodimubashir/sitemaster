            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" class="forms-sample material-form"
                        ...
                        <!-- Item Name Toggle -->
                        @php
                            $itemExistsInList = $items->contains('item_name', $construction_material_billing->item_name);
                        @endphp
                        
                        <div class="mb-4">
                            <div class="btn-group btn-group-sm mb-2" role="group">
                                <button type="button" class="btn btn-outline-primary toggle-item-btn {{ $itemExistsInList ? 'active' : '' }}" data-mode="select">
                                    Select from list
                                </button>
                                <button type="button" class="btn btn-outline-secondary toggle-item-btn {{ !$itemExistsInList ? 'active' : '' }}" data-mode="custom">
                                    Enter custom
                                </button>
                            </div>
                        ...
                        <!-- Hidden Fields -->
                        <input type="hidden" name="site_id" value="{{ $construction_material_billing->phase->site->id }}" />
                        <input type="hidden" name="phase_id" value="{{ $construction_material_billing->phase->id }}" />

                        <!-- Supplier -->
                        <div class="mt-4">
                            <label for="supplier_id" class="mb-1" style="font-size: 0.8rem; color: rgba(17, 17, 17, 0.48);">
                                Select Supplier
                            </label>
                            <select id="supplier_id" class="form-select form-select-sm text-black" name="supplier_id">
                                @foreach ($suppliers as $supplier)
                                    <option {{ $construction_material_billing->supplier_id === $supplier->id ? 'selected' : '' }}
                        ...
                        <!-- Image Upload -->
                        <div class="mt-4">
                            <label for="image" class="mb-1" style="font-size: 0.8rem; color: rgba(17, 17, 17, 0.48);">
                                Item Bill
                            </label>
                            ...
                            @if($construction_material_billing->item_image_path)
                                <div class="mt-2">
                                    ...
                                    <a href="{{ asset('storage/' . $construction_material_billing->item_image_path) }}" target="_blank">
                                        View Image
                                    </a>
                                </div>
                            @endif
