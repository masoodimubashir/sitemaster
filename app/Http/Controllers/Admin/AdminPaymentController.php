...
class AdminPaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = $data->merge($sites->flatMap(function ($site) {
            $received = $site->adminPayments->where('transaction_type', 0)->sum('amount');
            $sent = $site->adminPayments->where('transaction_type', 1)->sum('amount');
        ...
        $data = $data->merge($suppliers->flatMap(function ($supplier) {
            $received = $supplier->adminPayments->where('transaction_type', 0)->sum('amount');
            $sent = $supplier->adminPayments->where('transaction_type', 1)->sum('amount');
